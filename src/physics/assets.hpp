...
public:
    ...
    // Return mesh asset for path/scale. If normalized=true, asset is COM-centered & principal-axes aligned.
    const MeshAsset& getMesh(const std::string& path,
                             const Vector3r& scale = Vector3r::Ones(),
                             bool normalized = false) const;

    const HeightFieldAsset& getHeightField(const std::string& exrPath,
                                           real_t x_dim,
                                           real_t y_dim,
                                           real_t z_scale = (real_t)1.0,
                                           real_t min_height = (real_t)0.0) const;
