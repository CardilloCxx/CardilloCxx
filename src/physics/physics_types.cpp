...
RigidState::RigidState(const Vector3r& p_local,
    ...
    if (refEntity != entt::null &&
        reg.all_of<World::C_Position3, World::C_Orientation, World::C_LinearVelocity3, World::C_AngularVelocity3>(refEntity)) {
        const auto& r_ORef = reg.get<World::C_Position3>(refEntity).value;
        const auto& q_Ref = reg.get<World::C_Orientation>(refEntity).value;
        const auto& v_Ref = reg.get<World::C_LinearVelocity3>(refEntity).value;
        const auto& w_Ref = reg.get<World::C_AngularVelocity3>(refEntity).value;
