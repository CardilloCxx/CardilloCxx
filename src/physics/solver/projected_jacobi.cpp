...
static inline VectorXr precompute_Rdiag_sparse(const Eigen::SparseMatrix<real_t, Eigen::RowMajor>& W, const VectorXr& MinvDiag, real_t alpha) {
...
static inline PJIterContext build_context(cardillo::physics::DynamicsAssembler& dyn, cardillo::physics::assembly::PjAssembler& m_assembler, real_t alpha) {
...
// Project per-contact groups: 1-row -> unilateral; 3-rows -> Coulomb cone (disk in tangential
// plane)
static inline void project_groups(const PJIterContext& ctx, const VectorXr& z, const VectorXr& p_old, VectorXr& p_proj) {
    ...
    const auto& groups = ctx.contactRowGroups;
    const auto& mus = ctx.groupMu;
...
static inline void relax_exchange_update(PJIterContext& ctx, const VectorXr& x_old, const VectorXr& p_old, VectorXr& x_new, VectorXr& p_new) {
    const auto& W = ctx.W;
    cardillo::physics::DynamicsAssembler* dyn = ctx.dyn;

    // Use effective-mass solve (S^{-1}) to update u
    // Build extended RHS [tmp_concat; 0; 0] and solve full S * delta_x = rhs_ext, then apply only
    // velocity part
...
static inline real_t global_segment_norm(const PJIterContext& ctx, const VectorXr& a, const VectorXr& b, real_t eps_abs) {
    ...
    for (int br = 0; br + 1 < (int)bo.size(); ++br) {
        int off = bo[(size_t)br];
        int n = bo[(size_t)br + 1] - off;
        if (n > 0) {
            Eigen::Map<const VectorXr> va(a.data() + off, n), vb(b.data() + off, n);
            if (ctx.eps_rel > (real_t)0) {
                const VectorXr scale = VectorXr::Constant(n, eps_abs) + ctx.eps_rel * va.cwiseAbs().cwiseMax(vb.cwiseAbs());
...
static inline void standard_loop(PJIterContext& ctx, VectorXr& u, VectorXr& p) {
...
static inline void nesterov_loop(PJIterContext& ctx, VectorXr& u, VectorXr& p, double beta_threshold, int restart_limit) {
    ctx.err_global = std::numeric_limits<real_t>::max();
    ctx.iteration = 0;
    VectorXr xuk = u;
    VectorXr xpk = p;
    ...
    while (ctx.iteration < ctx.maxIterations) {
        ...
        if (!std::isfinite((double)ctx.err_global)) {
            restart = true;
            resetReason = "nan_error";
        } else if (err_prev < std::numeric_limits<real_t>::infinity() && ctx.err_global > (real_t)1.05 * err_prev) {
            restart = true;
            resetReason = "error_increase";
        }
        if (!restart && betak1 > beta_threshold) {
            restart = true;
            resetReason = "beta_threshold";
        }
        if (!restart) {
            VectorXr a = yuk - xuk1;
            VectorXr b = xuk1 - xuk;
            ...
            if (d > 0.0) {
                restart = true;
                resetReason = "direction_conflict";
            }
        }
        if (!restart && (!std::isfinite(betak1) || betak1 < 0.0 || betak1 > 1.0)) {
            restart = true;
            resetReason = "beta_invalid";
        }
        if (ctx.err_global <= (real_t)1) break;
        ...
        if (ctx.debug && (ctx.iteration % 1000 == 0)) {
            std::cout << "[ProjectedJacobi+Nesterov] Iter " << ctx.iteration << ", error = " << ctx.err_global << ", beta = " << betak1 << std::endl;
        }
    }
