...
class DynamicsAssembler {
   public:
    ...
    const VectorXr& restitutionVec() const { return m_restitution_vec; }
   ...
   private:
    ...
    VectorXr m_restitution_vec;  // restitution coefficients for each contact row (size = num contact rows)
