...
public:
    ...
    public:
        ...
        ~Scope() { if (m_active && m_mgr) m_mgr->end(); }
    private:
    ...
    const Stats& stats(TimerId id) const {
        auto it = m_stats.find(id); if (it != m_stats.end()) return it->second;
        static Stats empty; return empty;
    }
    ...
    void printBreakdown(std::ostream& os) const {
        ...
        struct Row { std::string name; double sec; std::uint64_t calls; double pct; double avg; };
        std::vector<Row> rows; rows.reserve(m_stats.size() + 1);
        for (auto& kv : m_stats) {
            if (kv.first == TimerId::Total || kv.first == TimerId::Other) continue;
        ...
        for (auto& r : rows) {
            os << std::left << std::setw(24) << r.name
               ...
               << std::setw(12) << std::fixed << std::setprecision(6) << r.sec
               << std::setw(10) << std::setprecision(2) << r.pct
               << std::setw(12) << std::setprecision(2) << r.avg * 1e6 << "\n";
        }
        os << std::string(24+10+12+10+12, '-') << "\n";
        os << std::left << std::setw(24) << "Total (inclusive)" << std::right << std::setw(10) << "" << std::setw(12) << std::fixed << std::setprecision(4) << grand << std::setw(10) << 100.0 << std::setw(12) << "" << "\n";
...
private:
    ...
    void begin(TimerId id) {
        m_stack.push_back({id, clock::now(), 0.0});
    }
    void end() {
    ...
    void printNode(std::ostream& os, const HierNode& node, int depth, double grand) const {
        ...
        os << std::string(depth * 2, ' ') << "- " << nameOf(node.id)
           << " (incl=" << std::fixed << std::setprecision(6) << node.inclusiveSec
           << "s, excl=" << node.exclusiveSec << "s, " << std::setprecision(2) << pct << "%)\n";
    ...
    static std::string nameOf(TimerId id) {
        switch (id) {
            case TimerId::CollisionBroadphase: return "Collision Broadphase"; 
            case TimerId::CollisionMatching: return "Collision Matching"; 
            case TimerId::CollisionNarrowphase: return "Collision Narrowphase"; 
            ...
            case TimerId::Other: return "Other"; 
        }
        return "Unknown";
    }
