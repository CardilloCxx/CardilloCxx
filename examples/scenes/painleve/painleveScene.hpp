...
public:
    ...
    void populate(cardillo::PhysicsSystem& sys) override {
        ...
        const real_t friction = (real_t)1.66666;
        ...
        const real_t friction_min = 0.5;                    const real_t friction_max = 4.0;     
        
        const size_t num_phi_steps = 100;
        const real_t phi_min =  (real_t)(20 * M_PI / 180.0); const real_t phi_max = (real_t)(90 * M_PI / 180.0);
        ...
        for (size_t i = 0; i < num_friction_steps; ++i) {
            ...
            for (size_t j = 0; j < num_phi_steps; ++j) {
                ...
                const Vector3r position(x, y, l * std::sin(curr_phi) + 1.0 * thickness * std::cos(curr_phi));
                PhysicsSystem::CapsuleShape rodShape{thickness * (real_t)0.5, l};
