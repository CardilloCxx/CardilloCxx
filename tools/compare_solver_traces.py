...
TRACE_RE = re.compile(r"Simulating\s+\d+%\|.*?\|(\s*)(\d+)/(\d+).*?iters=(\d+)")
...
def run_case(repo_root: Path, bin_path: Path, case: Case, run_dir: Path, timeout_s: int) -> tuple[list[int], list[int], Path]:
    ...
    for line in output.splitlines():
        ...
        if match:
            ...
            iters.append(int(match.group(4)))

    return steps, iters, log_path


def worker(repo_root: Path, bin_path: Path, case: Case, log_root: Path, timeout_s: int) -> tuple[Case, list[int], list[int], Path]:
    print(f"[START] Running {case.scene} / {case.label} ...")
    steps, iters, log_path = run_case(repo_root, bin_path, case, log_root, timeout_s)
    print(f"[FINISH] {case.scene} / {case.label} captured {len(steps)} points.")
    return case, steps, iters, log_path
...
def plot_combined_results(
    ...
    for ax, scene in zip(axes, unique_scenes, strict=True):
        ...
        for solver_type in ("pj", "pgs", "clarabel"):
        ...
        for solver_type in ("pj", "pgs", "clarabel"):
            for nesterov in (False, True):
                ...
                iters = np.array([it if it > 0 else 1 for it in iters], dtype=float)
                
                label = f"{solver_type.upper()} {'Nesterov' if nesterov else 'Plain'}"
                color = colors[(solver_type, nesterov)]

                # Calculate sigma relative to the percentage of the dataset's total span
                if smoothness_fraction > 0.0 and len(steps) > 1:
                    avg_spacing = np.mean(np.diff(steps))
                    
                    if avg_spacing > 0:
                        # Desired physical window width in X-units
                        target_window = max_steps * smoothness_fraction
                        # Convert to index-space sigma
...
def main() -> int:
    ...
    bin_path = repo_root / "build" / "bin" / "main"
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = [
            executor.submit(worker, repo_root, bin_path, case, log_root, args.timeout)
            for case in cases
        ]
        
        for future in futures:
            case, steps, iters, log_path = future.result()
            results[(case.scene, case.label)] = (steps, iters)
            
            for step, it in list(zip(steps, iters, strict=True))[:10]:
                summary_rows.append({"scene": case.scene, "case": case.label, "step": step, "iters": it})

    summary_csv = output_dir / "trace_summary.csv"
    with summary_csv.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["scene", "case", "step", "iters"])
        writer.writeheader()
